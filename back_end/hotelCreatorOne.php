...
//$con->close();
	...
	if(mysqli_num_rows($hotelcheckI) >0 && mysqli_num_rows($hotelcheckJ) > 0){
		...
		$_SESSION["ErrorCone"] = "hotel already exists"; // error code 3 name exists cannot register
...
(`id`, `hotelChain`, `hotelLocationI`, `hotelLocationJ`, `numLuxury`, `numStandard`, `priceLuxury`, `priceStandard`)
VALUES (NULL, '$hotelChain', '$hotelLocationI', '$hotelLocationJ', '$numLux','$numStan', '$priceLux', '$priceStan')";
...
<html>
<head>
    <title>Chain 1 Hotel Creator</title>
...
?>
    ...
    <form action="" method="post" class="form">
        ...
        <?php
        if(isset($_SESSION["ErrorCone"])){ // checks if there is an error message set
        echo '<p class = "statusMessage">' .$_SESSION["ErrorCone"].'</p>'; // Outputs error message to user
    }
        ?>
    </form>
