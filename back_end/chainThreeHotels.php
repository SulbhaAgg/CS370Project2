...
<table class="table table-dark">
  ...
  <tbody>
 <?php
    $chainCheckQuery = "SELECT id,hotelChain, hotelLocationI, hotelLocationJ, numLuxury, priceLuxury, numStandard, priceStandard FROM `hotel` WHERE hotelChain = 'Chain 3'";
    $chainCheck = mysqli_query($con,$chainCheckQuery)or die("Well that didn't work");
    $num_results = mysqli_num_rows($chainCheck);

    for($i = 1; $i <= $num_results; $i++ ){
      echo '<form action="chainThreeBook.php" method="post">';
      echo '<tr>';
